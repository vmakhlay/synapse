...
IF NOT EXISTS
   (  SELECT [name]
      FROM sys.tables
      WHERE [name] = 'e2e_cross_signing_signatures'
   )
   CREATE TABLE e2e_cross_signing_signatures (
        ...
        key_id TEXT NOT NULL,
        -- user who was signed
        target_user_id NVARCHAR(4000) NOT NULL,
        -- device/key that was signed
        target_device_id NVARCHAR(4000) NOT NULL,
        -- the actual signature
        signature TEXT NOT NULL
   );
...
IF NOT EXISTS
   (  SELECT [name]
      FROM sys.tables
      WHERE [name] = 'user_signature_stream'
   )
